...
namespace WebGL.UI.ProceduralIcons
{
    ...
    public partial class ProceduralResetIcon : ProceduralIconBase
    {
        ...
        protected override bool UpdateCustomPhysics(float dt)
        {
            ...
            if (Mathf.Abs(currentRotationAngle - targetRotationAngle) < 0.1f)
            {
                NormalizeRotationState();
            }
        ...
        private void NormalizeRotationState()
        {
            if (Mathf.Abs(currentRotationAngle) < 720f && Mathf.Abs(targetRotationAngle) < 720f)
            {
                return;
            }
