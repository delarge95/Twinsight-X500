...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{p{2.8cm}p{2.8cm}Xp{2.4cm}}
        \hline
        \textit{Frente} & \textit{Estado} & \textit{Evidencia de cierre} & \textit{Dato de validación} \\ \hline
        Flujo principal WebGL & QA funcional correcto & La app permite explorar, seleccionar, aislar y analizar sin romper flujo & Métricas por escenario \\
        Interacción móvil & QA P1/P2 correcto & Gestos, \textit{bottom sheet}, doble toque, zoom e iconos ajustados & Tabla por dispositivo \\
        Rendimiento percibido & Correcto en revisión & La app se observó fluida en escritorio y en la mayoría de móviles & FPS, memoria y carga \\
        Usabilidad formativa & Correcta para cierre & La prueba breve permitió convertir gestos en ajustes de interfaz & SUS, NASA-TLX y \textit{Think-Aloud} \\
        Manuales e informe & Coherencia revisada & La documentación distingue funciones visibles y capacidades ocultas & Datos de campo registrados \\ \hline
    \end{tabularx}
    \apanote[10.6pt]{Esta tabla resume el cierre funcional y no reemplaza las tablas cuantitativas posteriores. Elaboración propia.}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{p{3.1cm}Xp{3.8cm}p{2.1cm}}
        \hline
        \textit{Necesidad industrial} & \textit{Respuesta actual del proyecto} & \textit{Límite vigente} & \textit{Madurez} \\ \hline
        Comprensión espacial & Navegación 3D, selección, aislamiento, vista explosionada y corte & Se complementa con documentación técnica & Visual product twin \\
        Transferencia técnica & Onboarding, hotspots, fichas por pieza y flujo mobile-first & Falta medir formalmente aprendizaje longitudinal & Visual product twin \\
        Conversión CAD a web & Pipeline CAD/Blender/Unity, saneamiento runtime y profiler & Reducción geométrica reportada por rutas & Digital model \\
        Datos fragmentados & Taxonomía 28/30/257 y fichas como base de identificadores & No existe aún \textit{twin manifest} formal & Preparación semántica \\
        Soporte y mantenimiento & Selección contextual y lectura por componentes como base & No hay repuestos ni reglas de diagnóstico & Trabajo futuro \\
        Estado operacional & Visualización térmica heurística por componente & No hay telemetría real ni sincronización & Pre-shadow conceptual \\ \hline
    \end{tabularx}
    \apanote[14.4pt]{La tabla distingue respuesta funcional actual, límite técnico y nivel de madurez. No clasifica la build como \textit{digital twin} operacional. Elaboración propia a partir de la investigación conceptual y la build final.}
\end{table}

\begin{figure}[htbp]
    \caption{Necesidades Industriales, Respuesta Actual y Evolución Futura Defendible}
    \label{fig:necesidades_respuesta_evolucion}
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/apa_redesign/fig_apa_necesidades_respuesta_evolucion.pdf}
...
\input{validacion/07_TABLAS_RENDIMIENTO_WEBGL_MEDICIONES}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{p{2.4cm}p{2.6cm}p{2.8cm}p{1.7cm}X}
        \hline
        \textit{Factor de forma} & \textit{Navegador} & \textit{Compatibilidad} & \textit{Validado} & \textit{Observaciones} \\ \hline
        Escritorio & Windows 11 / Chrome 141 & Compatible & Sí & Fluidez estable en escenarios base, selección, explode y corte. \\
        Móvil Android gama media & MIUI 14 / Chrome 148 & Compatible con restricciones & Sí & Usable con picos perceptibles; referencia Redmi Note 10S. \\
        Móvil Android límite inf. & Android 12 / Chrome 114 & Compatible con restricciones & Sí & Navegable, por debajo de la meta de 30 FPS. \\
        Móvil Android gama alta & Android 12 / Chrome 114 & Compatible & Sí & Cercano a 30 FPS promedio y estable. \\
        Móvil iOS gama alta & iOS / Safari & Compatible & Sí & Fluidez cercana a 60 FPS en recorrido completo. \\
        Captura descartada & Android 12 / Chrome 91 & No usada & No & Sesión separada por pausa atípica del navegador. \\ \hline
    \end{tabularx}
    \apanote[6.1pt]{La tabla resume compatibilidad observada por factor de forma y navegador a partir de las sesiones del profiler interno. Elaboración propia.}
\end{table}
...
\begin{figure}[htbp]
    ...
    \apanote[+14.4pt]{La matriz sintetiza dispositivo, sistema operativo, navegador, resolución, FPS promedio y lectura operativa de las sesiones técnicas registradas. Elaboración propia.}
\end{figure}
...
\paragraph{Gestos en Tutorial} Deslizar tarjetas del tutorial.

\paragraph{Navegación por Paneles} Arrastrar verticalmente el panel inferior.

\paragraph{Sensibilidad de Cámara} Controlar el zoom con menor sensibilidad.

\paragraph{Aislamiento Directo} Aislar piezas sin que apareciera de inmediato un panel de información.

\paragraph{Doble Toque} Usar doble toque sobre piezas u hotspots.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{p{2.8cm}p{3.8cm}Xp{1.8cm}}
        \hline
        \textit{Observación} & \textit{Comportamiento detectado} & \textit{Corrección aplicada} & \textit{Estado} \\ \hline
        Tutorial & Usuarios intentaron hacer \textit{swipe} & Navegación por dots/gestos y cierre descendente & QA correcto \\
        Panel de información & Gesto se confundía con el dron & El panel bloquea su área táctil & QA correcto \\
        Zoom y órbita & Sensibilidad táctil percibida alta & Reducción de sensibilidad en móvil & QA correcto \\
        Aislamiento & Abrir panel al aislar resultó agresivo & Doble toque aísla/desaísla sin abrir panel & QA correcto \\
        Hotspots & Se esperaba doble toque sobre hotspots & Doble tap para aislamiento de hotspots & QA correcto \\
        Botón Atrás & Salida indeseada del navegador & Navegación por capas y confirmación & QA correcto \\
        Iconos móviles & Microinteracciones en estado incorrecto & Estabilización de estados táctiles & QA correcto \\ \hline
    \end{tabularx}
    \apanote[14.4pt]{Estos hallazgos provienen de observación formativa previa y explican la evolución P1/P2 aplicada antes de consolidar los instrumentos de usuario. Elaboración propia.}
\end{table}

\begin{figure}[htbp]
    \caption{Mapa de Gestos Espontáneos y Correcciones P1/P2}
    \label{fig:mapa_gestos_fixes}
    \includegraphics[width=0.88\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter5/fig_5_mapa_gestos_fixes.pdf}
    \apanote[6.1pt]{Mapeo visual que conecta las fricciones de interacción táctil identificadas en la prueba previa de Think-Aloud con sus correspondientes soluciones implementadas en la interfaz de la build. Elaboración propia.}
\end{figure}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{p{3.1cm}p{2.2cm}p{2.2cm}p{2.2cm}X}
        \hline
        \textit{Tarea} & \textit{Visor 3D} & \textit{Soporte 2D} & \textit{Errores / ayudas} & \textit{Lectura} \\ \hline
        T1. Ubicación & 12/12 completadas & 12/12 completadas & 0 errores; 0 ayudas & Localizaron el motor y piezas cercanas. \\
        T2. Interpretación & 12/12 completadas & 12/12 completadas & 0 errores; 1 ayuda & El \textit{bottom sheet} permitió consultar información. \\
        T3. Relación espacial & 12/12 completadas & 12/12 completadas & 0 errores; 0 ayudas & Relación motor--montura--tornillería resuelta. \\
        T4. Análisis guiado & 12/12 completadas & 12/12 completadas & 0 errores; 0 ayudas & En 3D se usaron explode, X-Ray y presets. \\ \hline
    \end{tabularx}
    \apanote[14.4pt]{La tabla sintetiza los registros estructurados del moderador por tarea y condición. La lectura cualitativa complementaria se presenta en la sección de \textit{Think-Aloud}. Elaboración propia.}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{p{1.2cm}p{1.0cm}p{1.8cm}>{\raggedleft\arraybackslash}X>{\raggedleft\arraybackslash}X>{\raggedleft\arraybackslash}X>{\raggedleft\arraybackslash}X>{\raggedleft\arraybackslash}X>{\raggedleft\arraybackslash}X}
        \hline
        \textit{Código} & \textit{Sec.} & \textit{Disp.} & \textit{T1 3D} & \textit{T1 2D} & \textit{T2 3D} & \textit{T2 2D} & \textit{T3 3D} & \textit{T3 2D} \\ \hline
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{p{2.8cm}>{\raggedleft\arraybackslash}X>{\raggedleft\arraybackslash}X>{\raggedleft\arraybackslash}X>{\raggedleft\arraybackslash}Xp{4.5cm}}
        \hline
        \textit{Tarea} & \textit{Media 3D} & \textit{Media 2D} & \textit{Dif. 2D--3D} & \textit{n} & \textit{Lectura} \\ \hline
        ...
        \textit{Total T1--T3} & \textit{20,58 s} & \textit{54,00 s} & \textit{33,42 s} & \textit{12} & \textit{Menor duración acumulada en 3D.} \\ \hline
    \end{tabularx}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{p{1.2cm}p{0.9cm}p{1.6cm}p{0.9cm}p{1.3cm}p{1.3cm}p{1.3cm}X}
        \hline
        \textit{Código} & \textit{Sec.} & \textit{Dispositivo} & \textit{SUS} & \textit{NASA 3D} & \textit{NASA 2D} & \textit{Dif.} & \textit{Lectura} \\ \hline
    ...
    \end{tabularx}
    \apanote[+28.8pt]{Sec. = secuencia de exposición; AB = visor 3D primero y soporte 2D después; BA = soporte 2D primero y visor 3D después. La diferencia se calcula como NASA-TLX Raw 2D menos NASA-TLX Raw 3D. Elaboración propia a partir de los instrumentos de validación aplicados.}
\end{table}

\begin{table}[htbp]
    \caption{Resumen Descriptivo de Usabilidad y Carga de Trabajo Percibida}
    \label{tab:validacion_resumen_sus_nasa}
    \begin{tabularx}{\textwidth}{p{5.0cm}>{\raggedleft\arraybackslash}X>{\raggedleft\arraybackslash}X>{\raggedleft\arraybackslash}X>{\raggedleft\arraybackslash}X>{\raggedleft\arraybackslash}X>{\raggedleft\arraybackslash}X}
        \hline
        \textit{Medida} & \textit{n} & \textit{Media} & \textit{Mediana} & \textit{Mín.} & \textit{Máx.} & \textit{DE} \\ \hline
    ...
    \end{tabularx}
    \apanote[+3.7pt]{DE = desviación estándar. Los resultados se interpretan de forma descriptiva y exploratoria por el tamaño de muestra y el muestreo no probabilístico. Elaboración propia.}
\end{table}

\begin{figure}[htbp]
    \caption{Distribución del Puntaje SUS en la Muestra Evaluada}
    \label{fig:distribucion_sus}
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter5/fig_5_sus_distribucion.pdf}
    \apanote[+6.2pt]{La línea de referencia marca 68 puntos, valor usado como promedio histórico contextual del instrumento. La lectura del proyecto es descriptiva y no inferencial. Elaboración propia.}
\end{figure}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{X>{\raggedleft\arraybackslash}p{2.4cm}>{\raggedleft\arraybackslash}p{2.4cm}>{\raggedleft\arraybackslash}p{2.4cm}}
        \hline
        \textit{Dimensión} & \textit{Visor 3D} & \textit{Soporte 2D} & \textit{Diferencia} \\ \hline
    ...
    \end{tabularx}
    \apanote[3.7pt]{La dimensión de rendimiento se registró con escala invertida: 0 = perfecto y 100 = fallido. Por ello, valores menores indican mejor percepción de desempeño. Elaboración propia.}
\end{table}

\begin{figure}[htbp]
    \caption{Comparativa NASA-TLX Raw por Dimensiones}
    \label{fig:nasatlx_dimensiones}
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter5/fig_5_nasatlx_dimensiones.pdf}
    \apanote[14.4pt]{La figura compara los promedios por dimensión para el visor 3D y el soporte 2D de referencia. Valores menores representan menor carga de trabajo percibida. Elaboración propia.}
\end{figure}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{p{3.5cm}p{1.9cm}Xp{2.4cm}}
        \hline
        \textit{Categoría analítica} & \textit{Frecuencia} & \textit{Evidencia representativa} & \textit{Lectura} \\ \hline
        Comprensión espacial & 11/12 & Relación motor, montura, tornillería y conjunto & Fortaleza principal. \\
        Navegación y control & 10/12 & Fricciones con órbita, \textit{pan} y sensibilidad táctil & Punto de mejora UX móvil. \\
        Percepción de claridad & 8/12 & Explosionada, térmica, X-Ray, Blueprint y aislamiento & Apoya menor carga de trabajo. \\
        Interpretación de UI & 7/12 & Lectura de iconos y menús concentró dudas & Pulido visual continuo. \\
        Estrategias de recuperación & 3/12 & Cambios de ángulo o gestos repetidos & Aprendizaje durante la sesión. \\
        Errores o bloqueos & 2/12 & Selección de piezas pequeñas generó fricción & Riesgo localizado. \\ \hline
    \end{tabularx}
    \apanote{La frecuencia indica cantidad de participantes en los que apareció la categoría al menos una vez. Elaboración propia.}
\end{table}

\begin{figure}[htbp]
    \caption{Mapa de Codificación Cualitativa del Protocolo \textit{Think-Aloud}}
    \label{fig:thinkaloud_codificacion}
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter5/fig_5_thinkaloud_codificacion.pdf}
    \apanote[+6.2pt]{El mapa sintetiza frecuencia por categoría y lectura interpretativa de las verbalizaciones y observaciones registradas durante la prueba. Elaboración propia.}
\end{figure}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{p{3.7cm}p{2.6cm}X}
        \hline
        \textit{Método} & \textit{Triángulos totales} & \textit{Detalle técnico observado} \\ \hline
        Importación directa & 6\,717\,499 tris & Malla caótica, conexiones en T, vértices y caras repetidas. \\
        MoI3D & 6\,864\,586 tris & Malla más limpia, pero con n-gons y vértices no soldados. \\
        STEPper & 6\,525\,748 tris & Menor cantidad de vértices repetidos, pero malla desordenada. \\
        Modelo optimizado & 95\,617 tris & Conteo del activo base optimizado y sus masters principales. \\ \hline
    \end{tabularx}
    \apanote[3.7pt]{La cifra de 95\,617 triángulos corresponde al modelo base optimizado exportado para la build. No equivale al conteo de escena runtime reportado por el profiler. Elaboración propia.}
\end{table}

\begin{figure}[htbp]
    \caption{Malla Optimizada Final y Lectura de Wireframe}
    \label{fig:resultados_geometria_malla_optimizada}
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_cad_bake_low_pair.png}
    \apanote[+14.4pt]{La comparación muestra el modelo optimizado y su equivalente en wireframe. Esta evidencia se ubica junto al conteo del activo base optimizado de 95\,617 triángulos para relacionar reducción poligonal, limpieza topológica y legibilidad visual del activo. Elaboración propia.}
\end{figure}

\begin{figure}[htbp]
    \caption{Renders del Dron Optimizado con Mapas de Normales y Oclusión Ambiental}
    \label{fig:renderes_caso_holybro_x500}
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_render_optimized_drone_stack.png}
    \apanote[6.2pt]{Los renders corresponden al modelo low poly optimizado y permiten verificar que la reducción geométrica conserva una lectura formal adecuada mediante apoyo de mapas de normales y AO. Elaboración propia.}
\end{figure}
