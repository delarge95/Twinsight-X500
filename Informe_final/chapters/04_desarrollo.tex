...
\paragraph{Ejecutado} Procesos, herramientas y flujos que dejaron huella verificable en la app, la escena, el código o los artefactos activos.

\paragraph{Evaluado} Rutas técnicas estudiadas, probadas parcialmente o utilizadas como soporte de decisión, aunque no constituyan el estado final consolidado.

\paragraph{Descartado o Reemplazado} Enfoques tempranos que ayudaron a tomar decisiones, pero ya no representan la versión defendible del producto.
...
\paragraph{28 Piezas Canónicas} De investigación reflejadas en los assets \texttt{DronePartData} generados y en el catálogo público de fichas bilingües.

\paragraph{30 Anchors de Escena} Las 28 piezas más \texttt{x500v2\_fastener\_group} y \texttt{x500v2\_misc\_group}.

\paragraph{257 Renderers y Colliders} Auditados en la escena final.

\paragraph{257 Assets Generados} En \texttt{Assets/Core/Data/X500V2Generated}.

\paragraph{Inventario de Scripts} Organizado en tres niveles de lectura: 95 archivos runtime propios, 103 archivos bajo \texttt{Assets} si se excluyen tests y 129 archivos \texttt{.cs} en total al incluir editor y plugins.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{p{2.4cm}p{3.1cm}p{2.0cm}X}
        \hline
        \textit{Conteo} & \textit{Nivel de lectura} & \textit{Estado} & \textit{Interpretación correcta} \\ \hline
        ...
        257 \textit{renderers} & Fragmentación geométrica y cobertura de escena & Canónico técnico & Describe la granularidad renderizable/auditable del ensamblaje final, no una nueva taxonomía semántica. \\
    ...
    \end{tabularx}
    \apanote[-18.4pt]{Los conteos pertenecen a capas y momentos distintos del proyecto; por ello no deben interpretarse como equivalentes. Elaboración propia a partir del catálogo canónico, la escena final y los activos generados.}
\end{table}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.88\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter4/fig_4_taxonomia_28_30_257.pdf}
    \apanote[+6.2pt]{Mapeo jerárquico que organiza las 28 piezas canónicas de la investigación en 30 anchors de escena en Unity y 257 renderers y colliders físicos en runtime. Elaboración propia.}
\end{figure}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{p{2.6cm}p{4.0cm}p{2.4cm}X}
        \hline
        \textit{Etapa} & \textit{Foco principal} & \textit{Estado} & \textit{Lectura para el informe} \\ \hline
        ...
        Cierre técnico & Build WebGL, QA P1/P2, profiler interno y validación empírica & Build funcional & El flujo visible quedó validado para entrega; las mediciones formales se reportan con datos fechados. \\ \hline
    \end{tabularx}
    \apanote[14.4pt]{Esta tabla resume la evolución del proyecto y distingue cifras o decisiones tempranas frente al estado técnico vigente. Elaboración propia a partir del código, la documentación activa y la propuesta corregida.}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{p{2.8cm}p{3.1cm}p{2.0cm}X}
        \hline
        \textit{Ruta técnica} & \textit{Uso principal} & \textit{Estado} & \textit{Interpretación} \\ \hline
        STEP $\rightarrow$ Blender directo & Inspección rápida de piezas y lectura geométrica inicial & Evaluado & Útil para abrir piezas o familias sencillas, pero no siempre ofrece control de teselación. \\
        STEP $\rightarrow$ STEPper & Importación CAD directa dentro de Blender con addon & Ejecutado & Aportó una ruta práctica para abrir archivos STEP sin salir del entorno de modelado. \\
        MoI3D $\rightarrow$ Blender & Conversión de superficies NURBS con control fino de salida & Ejecutado & Se utilizó para casos donde la importación directa producía mallas difíciles de reutilizar. \\
        Re-modelado & Reconstrucción de piezas con topología defendible para tiempo real & Ejecutado & Fue la ruta dominante para piezas críticas cuando la malla CAD no era apta para WebGL. \\
        Sustitución modular & Tornillería, arandelas y elementos similares de alta repetición & Ejecutado & Redujo redundancia geométrica y abrió la puerta a instanciación y proxies ligeros. \\ \hline
    \end{tabularx}
    \apanote[11.5pt]{La coexistencia de varias rutas refleja la necesidad de adaptar el pipeline a distintos tipos de geometría CAD. Elaboración propia.}
\end{table}

\begin{figure}[htbp]
    \caption{Flujo de Decisión para Importación y Optimización CAD a WebGL}
    \label{fig:flujo_decision_cad_webgl}
    \includegraphics[width=0.92\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter4/fig_4_flujo_decision_cad_webgl.pdf}
    \apanote[-18.4pt]{Rutas técnicas de importación evaluadas y ejecutadas para trasladar la geometría CAD a Blender y Unity, indicando criterios de optimización poligonal y control de teselación. Elaboración propia.}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_cad_just_imported_pair.png}
    \apanote[+6.3pt]{A la izquierda se observa el modelo importado de forma directa desde STEP a Blender; a la derecha se presenta su equivalente en \textit{wireframe}. Elaboración propia.}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_cad_ngons_pair.png}
    \apanote[+6.3pt]{A la izquierda se muestra el modelo convertido desde MoI3D sin triangular; a la derecha se evidencia la presencia de n-gons y estructura de malla que requiere saneamiento antes del uso en WebGL. Elaboración propia.}
\end{figure}
...
\paragraph{Modeling} La fase de modelado no consistió únicamente en remodelar sobre referencia CAD. Dependiendo de la pieza, se siguieron tres rutas: piezas inexistentes que debieron modelarse desde cero; piezas existentes cuya malla CAD convertida no era trabajable y requirió retopología (véase la Figura \ref{fig:retopologia_pipeline}); y piezas que conservaron su geometría base, pero necesitaron correcciones puntuales por problemas típicos de conversión STEP a malla, como vértices no soldados, vértices repetidos, conexiones poligonales en T, caras internas, normales inconsistentes, geometría suelta o triangulaciones inestables.

\paragraph{Automation} Esta fase conservó el sentido técnico del MAD-T original: reducir trabajo repetitivo mediante herramientas y operaciones automatizables sobre la malla. En el proyecto incluyó selección de aristas duras, tratamiento de bevels generales, control de costuras, limpieza de marcas problemáticas, normalización de bordes, uso de modificadores y preparación consistente de superficies hard-surface antes de la reducción y exportación.

\paragraph{Decimation} La reducción geométrica se aplicó cuando el detalle no aportaba legibilidad técnica o excedía el presupuesto esperado para WebGL. La decisión no fue eliminar detalle de forma indisicriminada, sino conservar silueta, lectura funcional y relaciones de ensamblaje mientras se disminuía el costo de vértices y triángulos.

\paragraph{Triangulation} La triangulación se entendió como cierre de producción: una forma de evitar que Blender, el importador de Unity o el runtime WebGL decidieran triangulaciones implícitas distintas. Esto permitió estabilizar sombreado, normales, UVs y comportamiento de exportación.
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.86\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter4/fig_4_madt_adaptado.pdf}
    \apanote[11.5pt]{La adaptación se usó como criterio técnico de producción hard-surface; no reemplaza la metodología académica DSRM del proyecto. Elaboración propia.}
\end{figure}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{@{}>{\RaggedRight\arraybackslash}p{2.4cm}>{\RaggedRight\arraybackslash}p{3.6cm}>{\RaggedRight\arraybackslash}p{3.8cm}Y@{}}
        \hline
        \textit{Familia o pieza} & \textit{Entrada o referencia} & \textit{Acción aplicada} & \textit{Salida runtime o canónica} \\ \hline
        Placas del frame & Piezas CAD del kit X500 & Limpieza, renombrado y adaptación & Anchors \path{x500v2_bottom_plate}, \path{x500v2_top_plate}. \\
        Brazos y soportes & Tubos de carbono y clamps & Agrupación e instanciación por cuadrante & Anchors \path{x500v2_arm_FL/FR/BL/BR}. \\
        Pixhawk 6C & Carcasa y PCB en CAD & Encapsulado en lectura semántica única & Anchor \path{x500v2_pixhawk6c}. \\
        GPS M10 & Antena, mástil y base & Unión semántica de subpartes & Anchor \path{x500v2_gps_m10}. \\
        Motores & Modelo base de motor & Instanciación del modelo base & Anchors \path{x500v2_motor_FL/FR/BL/BR}. \\
        Fasteners & Familias CAD GB70 & Clasificación y proxies ligeros & Grupo \path{x500v2_fastener_group}. \\ \hline
    \end{tabularx}
    \apanote[14.5pt]{La tabla resume casos representativos y no pretende listar cada subpieza del pipeline. Su función es hacer auditable la relación entre fuente geométrica, operación aplicada y destino runtime. Elaboración propia.}
\end{table}
...
\paragraph{Master} Malla limpia o saneada que funciona como referencia geométrica compartida de una familia repetida. Un master puede servir solo como fuente técnica para instancias o coincidir con una ocurrencia visible del ensamblaje cuando esa pieza también existe físicamente en el dron.

\paragraph{Instancia} Objeto que comparte datos de malla con un master, pero conserva transformación, posición, rotación y escala propias dentro del ensamblaje. Esto permite corregir una sola malla y propagar el cambio a todas sus ocurrencias.

\paragraph{Proxy} Representación geométrica ligera usada cuando una pieza no requiere detalle completo en reposo o cuando una familia repetitiva sería demasiado costosa si se conservara con su malla CAD original. El proxy conserva lectura morfológica y posición espacial, pero reduce vértices, peso y costo de inspección.
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter4/fig_4_automatizacion_blender_masters_instancias.pdf}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter4/fig_4_retopologia_pipeline.pdf}
    \apanote[6.2pt]{Diagrama conceptual del flujo de saneamiento geométrico aplicado para transformar geometría CAD en activos aptos para runtime WebGL. Elaboración propia.}
\end{figure}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{p{3.2cm}p{3.6cm}X}
        \hline
        \textit{Familia o caso} & \textit{Métrica de comparación} & \textit{Lectura técnica} \\ \hline
        Brazo o soporte crítico & Triángulos, vértices y normales. & Contrasta la geometría de origen frente a una versión apta para runtime sin perder silueta. \\
        Motor o carcasa compleja & Instancias y sombreado. & Muestra cuándo conviene conservar, limpiar o reconstruir una pieza. \\
        Fasteners repetitivos & Instancias y sustitución por proxy. & Evidencia el ahorro geométrico y semántico obtenido al agrupar sujetadores repetidos. \\
        Ensamble representativo & Costo geométrico agregado. & Relaciona el saneamiento con presupuesto WebGL, selección y comprensión visual. \\ \hline
    \end{tabularx}
    \apanote[+6.2pt]{La tabla define qué se compara; los valores cuantitativos se reportan en el capítulo de resultados con evidencia de la build evaluada. Elaboración propia.}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{p{3.4cm}p{2.6cm}X}
        \hline
        \textit{Componente} & \textit{Magnitud} & \textit{Función en el proyecto} \\ \hline
        Familias de fasteners & 20 & Agrupan sujetadores equivalentes para metadatos compartidos por tipo. \\
        Instancias registradas & 168 & Conservan la ubicación de cada ocurrencia concreta dentro del ensamblaje. \\
        Entradas de reconciliación & 9 & Resuelven diferencias entre nombres de escena, catálogo y metadatos. \\
        Reemplazo modular & Tornillos & Se aplicó como caso cerrado de reconstrucción modular y ahorro geométrico. \\ \hline
    \end{tabularx}
    \apanote[-18.4pt]{La modularización de fasteners integra datos, escena y optimización. Elaboración propia a partir de la escena y del runtime de fasteners del proyecto.}
\end{table}
...
\begin{figure}[htbp]
    ...
    \apanote[14.4pt]{La reconstrucción modular de tornillos se basa en combinaciones de cabeza, diámetro y segmentos de cuerpo para ajustar longitud sin conservar una malla CAD única por cada tornillo. Elaboración propia.}
\end{figure}

\begin{figure}[htbp]
    \caption{Ejemplo de Tornillo Generado Mediante Arreglo Modular}
    \label{fig:fasteners_modular_array}
    \includegraphics[width=0.82\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_fastener_modular_array.png}
    \apanote[14.4pt]{El ejemplo muestra un tornillo compuesto por una cabeza y siete segmentos repetidos de cuerpo, lo que ilustra el principio de variación de longitud mediante arreglo. Elaboración propia.}
\end{figure}

\begin{figure}[htbp]
    \caption{Sistema Modular Delimitado a Tornillos y Reconciliación de Fasteners}
    \label{fig:pipeline_modular_screw_only}
    \includegraphics[width=0.96\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter4/fig_4_pipeline_modular_screw_only.pdf}
...
En el flujo Blender--Unity, la escena runtime se exporta como unión de cuatro colecciones: masters bakeados, instancias del ensamblaje, masters primitivos de fasteners e instancias primitivas de fasteners. La decisión de exportar un master depende de su papel en el ensamblaje. El paquete de materiales previsto usa \texttt{BaseColor}, \texttt{Normal} y una máscara compacta \texttt{R=AO}, \texttt{G=Roughness}, \texttt{B=Curvature}, \texttt{A=Metallic}; la fibra de carbono se hornea desde el material construido en Blender. La asignación de fasteners a piezas madre se trata como revisión basada en manifest.
...
\begin{figure}[htbp]
    ...
    \makebox[\textwidth][c]{\includegraphics[width=0.96\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_fastener_original_proxy_stack.png}}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{p{2.0cm}p{2.8cm}p{2.0cm}p{1.65cm}X}
    \hline
    \textit{Herramienta} & \textit{Rol principal} & \textit{Licencia} & \textit{Rango} & \textit{Interpretación} \\ \hline
    Blender & Modelado, limpieza y exportación & Libre / gratuita & Sin costo & Núcleo ejecutado del pipeline de producción. \\
    Unity & Integración, UI, shaders y web & Plan académico & Sin costo & Punto de verdad del producto final. \\
    MoI3D & Conversión CAD & Comercial & Medio / alto & Apoyo para teselación con mayor control. \\
    STEPper & Importación STEP en Blender & Addon & Bajo / medio & Ruta directa para revisar piezas CAD. \\
    RizomUV & Despliegue y revisión UV & Comercial & Medio / alto & Apoyó la preparación de UVs para bake. \\
    Hard Ops / Box Cutter & Modelado y booleans & Addon comercial & Bajo / medio & Aceleró remodelado y corrección. \\
    Mesh Machine & Limpieza de bevels & Addon comercial & Bajo / medio & Se aprovechó para superficies duras. \\
    Marmoset Toolbag & Bake y lookdev puntual & Comercial & Medio / alto & Herramienta auxiliar del pipeline. \\ \hline
    \end{tabularx}
    \apanote[-18.4pt]{``Rango'' clasifica el tipo de inversión requerida sin fijar precios exactos sujetos a cambios comerciales. Elaboración propia.}
\end{table}

La Figura \ref{fig:software_marmoset_rizomuv} documenta el uso de Marmoset Toolbag y RizomUV como herramientas auxiliares del pipeline de \textit{baking}, \textit{lookdev} y despliegue UV. Estas capturas se agrupan entre sí porque representan software especializado de preparación visual y texturizado.

\begin{figure}[htbp]
    \caption{Software Auxiliar para \textit{Baking}, \textit{Lookdev} y Despliegue UV}
    \label{fig:software_marmoset_rizomuv}
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_software_marmoset_rizomuv.png}
    \apanote[+6.2pt]{La captura superior corresponde a Marmoset Toolbag como apoyo de \textit{baking} y revisión visual; la inferior corresponde a RizomUV como herramienta de despliegue y revisión UV. Elaboración propia.}
\end{figure}
...
El \textit{CAD Symmetry Instancer} se creó para detectar piezas repetidas, reconstruir relaciones de instancia y conservar transformaciones espaciales. Sus versiones posteriores incorporaron validación por nombre, volumen, conteo de vértices y registro por matrices.

El \textit{CAD Mesh Swapper} resolvió reemplazar la geometría interna de una pieza dañada por una versión limpia sin alterar su posición global. En paralelo, el \textit{CAD Fastener Proxy Automator} se orientó a tornillería y repetitivos, analizando proporciones de cabezal y caña para generar proxies de baja complejidad.

Finalmente, se desarrollaron scripts no destructivos para preparar el bake y la exportación final: creación de imágenes objetivo para mapas PBR, empaquetado de canales en máscaras runtime y generación de un manifest de Blender. La Figura \ref{fig:addons_pipeline_cad} sintetiza este soporte técnico de automatización.

\begin{table}[htbp]
    \caption{Automatizaciones y Addons Asociados al Pipeline}
    \label{tab:addons_automatizacion_pipeline}
    \begin{tabularx}{\textwidth}{p{3.0cm}p{3.2cm}p{2.8cm}X}
        \hline
        \textit{Herramienta} & \textit{Problema} & \textit{Salida técnica} & \textit{Estado e interpretación} \\ \hline
        CAD Symmetry Instancer & Piezas CAD repetidas como mallas únicas & Relación master/instancias & Ejecutado; central para reducir trabajo manual. \\
        Variante MoI3D ICP & Piezas con teselación irregular & Alineación por PCA/ICP & Evaluado; aporta una ruta para geometría irregular. \\
        CAD Mesh Swapper & Mallas dañadas o mal convertidas & Sustitución de geometría local & Ejecutado de forma puntual. \\
        Fastener Proxy Automator & Tornillería densa en reposo & Proxies ligeros por tipología & Ejecutado/evaluado; apoyó ahorro geométrico. \\
        Bake Target Setup & Preparación repetitiva para PBR & Targets de BaseColor, Normal, etc. & Documentado como apoyo no destructivo. \\
        Mask Packer & Exceso de texturas separadas & Máscara compacta \texttt{R=AO,G=Rough...} & Documentado para simplificar consumo en Unity. \\
        Runtime Manifest Exporter & Riesgo de perder masters o parentage & Manifest con roles y bounds & Ejecutado/documentado; hace auditable la escena. \\ \hline
    \end{tabularx}
    \apanote[14.4pt]{La documentación de estas herramientas se conserva como parte del desarrollo porque su valor está en la automatización del pipeline. Elaboración propia.}
\end{table}

\begin{figure}[htbp]
    \caption{Tooling Propio para Automatizar el Pipeline CAD}
    \label{fig:addons_pipeline_cad}
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter4/fig_4_addons_pipeline.pdf}
    \apanote[14.5pt]{La figura resume el soporte técnico de automatización del pipeline; no representa funciones visibles para usuarios finales de la aplicación. Elaboración propia.}
\end{figure}

La Figura \ref{fig:evidencia_bake_uv_fasteners_runtime} muestra el estado de alta resolución importado mediante STEPper y utilizado como fuente para el bake. Se presenta con vista sombreada y \textit{wireframe} apilados. Después, la Figura \ref{fig:evidencia_malla_optimizada_cap4} muestra la malla optimizada usada como referencia del cierre geométrico.

\begin{figure}[htbp]
    \caption{Modelo de Alta Resolución Usado como Fuente de Bake}
    \label{fig:evidencia_bake_uv_fasteners_runtime}
    \includegraphics[width=\textwidth,height=0.22\textheight,keepaspectratio]{figures/screenshots_contextual/fig_cad_bake_high_shaded_full.png}
    \par\vspace{0.2\baselineskip}
    \includegraphics[width=\textwidth,height=0.22\textheight,keepaspectratio]{figures/screenshots_contextual/fig_cad_bake_high_wireframe_full.png}
    \apanote[-24.5pt]{La vista sombreada y su equivalente \textit{wireframe} evidencian la densidad geométrica usada como fuente de bake frente al activo optimizado. Elaboración propia.}
\end{figure}

\begin{figure}[htbp]
    \caption{Malla Optimizada del Dron Después del Cierre Geométrico}
    \label{fig:evidencia_malla_optimizada_cap4}
    \includegraphics[width=\textwidth,height=0.22\textheight,keepaspectratio]{figures/screenshots_contextual/fig_cad_bake_low_shaded_full.png}
    \par\vspace{0.2\baselineskip}
    \includegraphics[width=\textwidth,height=0.22\textheight,keepaspectratio]{figures/screenshots_contextual/fig_cad_bake_low_wireframe_full.png}
...
\paragraph{Jerarquía Visual Clara} Para separar navegación primaria, acciones de modo y detalle contextual (Norman, 2013).

\paragraph{Agrupación por Proximidad, Similitud y Continuidad} Para evitar dispersión visual en paneles y tarjetas (Wertheimer, 1923; Köhler, 1929).

\paragraph{Soporte a la Lectura Técnica} Mediante color, contraste y consistencia visual (Ware, 2012).

\paragraph{Control de Navegación y Selección} En entornos 3D con foco en orientación y reducción de fricción operativa (Bowman et al., 2004; Darken \& Sibert, 1996).
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{p{3.2cm}p{4.8cm}X}
        \hline
        \textit{Criterio} & \textit{Aplicación en la app} & \textit{Interpretación} \\ \hline
        Jerarquía visual & Separación entre Hero, navegación inferior, modos y ficha inferior. & Permite que el usuario identifique el flujo principal sin menús complejos. \\
        ...
        Adaptación a escritorio & Reacomodo funcional del layout móvil sobre pantalla más amplia. & Utilizable para demostración y revisión técnica. \\
        Consistencia cromática & Paleta contenida, retroalimentación de selección y lectura por modos. & Refuerza la comprensión del sistema y evita ruido visual. \\ \hline
    \end{tabularx}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter4/fig_4_ui_evolution_mobile_first.pdf}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_ui_hero_mobile_pc.png}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_ui_explore_mobile_pc.png}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_ui_info_panel.png}
    \apanote[14.5pt]{La lámina muestra la pestaña que abre el panel de información y el panel contextual de pieza; se presenta como acercamiento de interfaz, no como captura completa de pantalla móvil. Elaboración propia.}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \apanote[-18.3pt]{El diagrama resume la arquitectura de implementación de los iconos procedurales y sus estados interactivos dentro de UI Toolkit. Elaboración propia.}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter4/fig_4_onboarding_runtime.pdf}
    \apanote[-18.3pt]{El diagrama resume la arquitectura de ayuda contextual generada por código; no depende de GIF, video ni spritesheets. Elaboración propia.}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.88\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_tutorial_cards_01.png}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.88\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_tutorial_cards_02.png}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.88\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_tutorial_cards_03.png}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.88\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_tutorial_cards_04.png}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.88\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_tutorial_cards_05.png}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.88\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter4/fig_4_1.pdf}
    \apanote[-18.4pt]{Organización de la aplicación en cuatro capas independientes: presentación (UI), orquestación (manager general y estados), servicios de escena (cámara, corte, explosión y térmico) y datos. Elaboración propia.}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.88\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter4/fig_4_2.pdf}
    \apanote[-18.4pt]{Diagrama del flujo de mensajes desacoplado entre emisores (UI y managers) y suscriptores de eventos mediante el bus central. Elaboración propia.}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/apa_redesign/fig_apa_app_state_machine.pdf}
    \apanote[+14.4pt]{Máquina de estados finitos que controla los estados del visor y las transiciones válidas durante la sesión del usuario. Elaboración propia.}
\end{figure}
...
\paragraph{Reparación del Modelo Importado} \texttt{ImportedDroneRuntimeBinder} corrige la jerarquía del dron importado en runtime. Su trabajo incluye: resolver anchors por \texttt{id}, reubicar objetos huérfanos de nivel superior o anidados, sintetizar grupos para fasteners y misceláneos, y reconstruir cachés de \texttt{ExplodedViewManager}, \texttt{PartVisibilityManager}, \texttt{PartCatalogManager}, \texttt{CrossSectionManager}, \texttt{ViewModeManager} y el subsistema térmico.
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter4/fig_4_6.pdf}
    \apanote[-18.3pt]{La figura resume la ruta de normalización y consumo de \texttt{DronePartData} en runtime, desde los activos importados y el catálogo hasta los controladores visibles de interfaz y escena. Elaboración propia.}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.88\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_ui_main_menus.png}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter4/fig_4_4.pdf}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.88\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_ui_hero_language.png}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.78\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_ui_hero_auxiliary.png}
    \apanote[+4.0pt]{De izquierda a derecha se muestran las pantallas \textit{About}, selección de dispositivo y salida de la app. Elaboración propia.}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \apanote[+4.2pt]{De izquierda a derecha se presentan los acercamientos de vista explosionada y filtros por categoría. Elaboración propia.}
\end{figure}

\begin{figure}[htbp]
    \caption{Panel de Control del Menú \textit{Studio}}
    \label{fig:evidencia_studio_panel_runtime}
    \includegraphics[width=0.48\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_ui_studio_panel.png}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.88\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_explore_hotspot_selection.png}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.88\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_explore_isolate_sequence.png}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.88\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_analyze_tool_outputs.png}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/apa_redesign/fig_apa_mapa_modos_visuales_proposito.pdf}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/apa_redesign/fig_apa_pipeline_shaders_modos.pdf}
    \apanote[14.5pt]{Diagrama del flujo de datos de renderizado y lógica de shaders aplicada por el motor para generar cada uno de los modos gráficos expuestos u ocultos. Elaboración propia.}
\end{figure}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{p{2.6cm}p{2.4cm}p{3.0cm}X}
        \hline
        \textit{Modo o preset} & \textit{Estado UI} & \textit{Rol visual} & \textit{Interpretación} \\ \hline
        Realistic & Visible & Estado base PBR & Referencia principal del visor para lectura material. \\
        X-Ray & Visible & Inspección interna & Aporta transparencia y lectura estructural. \\
        Solid View & Visible & Color plano y contorno & Útil para lectura formal y siluetas sin textura. \\
        Thermal & Visible & Análisis térmico & Simulación híbrida heurística, no FEA. \\
        Blueprint & Visible (preset) & Lectura diagramática & Se activa desde Studio; muestra líneas y grilla. \\
        Wireframe & Oculto & Lectura de malla & Capacidad implementada no publicada. \\
        Ghosted & Oculto & Visualización analítica & Capacidad técnica no expuesta. \\
        Day / Night & Visible & Variación ambiental & Afectan contraste y percepción volumétrica. \\
        Studio presets & Visible & Presets de estudio & Sirven para presentación y control visual. \\ \hline
    \end{tabularx}
    \apanote{La tabla distingue entre tarjetas visibles, presets publicados indirectamente y capacidades ocultas. Elaboración propia.}
\end{table}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_modes_direct_xray_solid_thermal.png}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_modes_studio_presets.png}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.82\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_modes_environment_color.png}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter4/fig_4_7.pdf}
    \apanote[-18.4pt]{La figura ubica el modo \textit{Thermal} como capa visual heurística por componentes, conectada al estado del dron y a un grafo térmico simplificado; no representa FEA, termografía ni temperatura real calibrada. Elaboración propia.}
\end{figure}

\begin{figure}[htbp]
    \caption{Lectura Visual del Modo \textit{Thermal} en la Build Final}
    \label{fig:thermal_bateria_motores_helices}
    \includegraphics[width=0.52\textwidth,height=0.45\textheight,keepaspectratio]{figures/screenshots_contextual/fig_thermal_single.png}
    \apanote[+6.2pt]{La captura muestra la convención visual usada para diferenciar componentes calientes y piezas pasivas dentro del modelo térmico heurístico. La leyenda orienta la lectura del gradiente sin representar termografía medida ni simulación FEA. Elaboración propia.}
\end{figure}
...
Hero y navegación principal; selección de piezas y \textit{bottom sheet}; hotspots; isolate; power y slider de carga; explode; cross-section; filtros por categoría; modos visuales \textit{X-Ray}, \textit{Solid View}/\textit{Solid Color} y \textit{Thermal}; presets de \textit{Studio}, incluyendo \textit{Blueprint}; thermal con leyenda visible.

\subsubsection{Implementado pero Oculto}

\texttt{MeasurementTool}; modos de vista \textit{Wireframe} y \textit{Ghosted} como capacidades implementadas pero no expuestas en la UI final; parte del flujo térmico avanzado y del clipping extendido.

\subsubsection{Código Legado o No Integrado}

\texttt{PartCatalogUI}; \texttt{SettingsPanel}; \texttt{LoadingController}; partes de \texttt{EnhancedInfoPanel}; suite de ensamblaje, BOM, anotaciones y connections.

\begin{table}[htbp]
    \caption{Inventario Sintético de Módulos y Estado de Integración}
    \label{tab:inventario_modulos_integracion}
    \begin{tabularx}{\textwidth}{p{3.0cm}p{4.2cm}p{2.8cm}X}
        \hline
        \textit{Categoría} & \textit{Componentes} & \textit{Estado} & \textit{Interpretación} \\ \hline
        Runtime principal & UI, selección, explode, clipping, view modes, preset \textit{Blueprint} y térmico & Expuesto o activo & Soporta el flujo visible y la arquitectura operativa. \\
        Capacidades ocultas & \texttt{MeasurementTool}, \textit{Wireframe}, \textit{Ghosted}, clipping extendido & Oculto & Capacidad existente no publicada al usuario. \\
        Legado no integrado & Catálogo legado, \texttt{SettingsPanel}, suite de ensamblaje & No integrado & Legado histórico o trabajo futuro. \\
        Tooling de editor & Wizard, auditoría de cobertura, constructor de grafo térmico & Soporte técnico & Trazabilidad del pipeline de desarrollo. \\ \hline
    \end{tabularx}
    \apanote[-18.4pt]{Esta tabla resume el criterio usado para separar funciones visibles, funciones ocultas y módulos no integrados. Elaboración propia.}
\end{table}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.86\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter4/fig_4_coherencia_documental.pdf}
    \apanote[6.2pt]{El criterio de cierre separa funciones visibles, capacidades ocultas, código legado y trabajo futuro para evitar contradicciones entre la build y la documentación. Elaboración propia.}
\end{figure}

\subsection{Tooling de Editor, Automatización y Verificación}

El proyecto conserva herramientas de editor y verificación que sí son defendibles como parte del desarrollo real: \texttt{ProjectSetupWizard}; \texttt{ImportedDroneCoverageAudit}; \texttt{ThermalContactGraphBuilderWindow}; y fixers de configuración y build WebGL.
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.88\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter4/fig_4_8.pdf}
    \apanote[-18.3pt]{Resumen de los cuellos de botella característicos de la plataforma WebGL (memoria lineal, draw calls, descargas de red) y las correspondientes optimizaciones de Technical Art. Elaboración propia.}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.82\textwidth,height=0.45\textheight,keepaspectratio]{figures/apa_redesign/fig_apa_deploy_pipeline.pdf}
    \apanote[-18.4pt]{Diagrama de flujo de integración continua, desde el empaquetado de assets hasta la publicación en el servidor de hosting y la evaluación de usuario. Elaboración propia.}
\end{figure}

\FloatBarrier
\subsection{Limitaciones y Frentes de Cierre}

Los siguientes frentes delimitan el alcance documentado y separan el cierre verificable de las iteraciones futuras. En primer lugar, la trazabilidad del pipeline: conservar la validación del FBX runtime final exportado desde Blender.

En segundo lugar, los materiales y texturas: documentar materiales finales o mantenerlos como mejora futura.

Adicionalmente, las auditorías jerárquicas: ejecutar rerun si se reemplaza la escena.

Asimismo, el audio: decidir si entra en iteración posterior o trabajo futuro.

Por otra parte, las métricas cuantitativas: conservar la trazabilidad de rendimiento, SUS y NASA-TLX.

También, el LOD nativo: decidir con profiling si se integra realmente.

En cuanto a la interfaz de escritorio: reconocer que es una adaptación funcional del sistema \textit{mobile-first}.

Respecto al twin manifest: formalizar como trabajo futuro un manifest por pieza.

Finalmente, la telemetría y el gemelo digital: mantener telemetría en vivo y simulación calibrada como fases posteriores.
...
\begin{figure}[htbp]
    \caption[Estado de Herramientas de Ensamblaje Proyectadas]{Estado de Herramientas de Ensamblaje Proyectadas o No Integradas por Módulo}
    \label{fig:herramientas_ensamblaje_proyectadas}
    \includegraphics[width=0.88\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter4/fig_4_9.pdf}
