% =============================================================================
% Capítulo 1 — Introducción (Variante para TOC corto)
% Secciones anteriores a Objetivos con asterisco para no figurar en el TOC.
...
\paragraph{Huella Inicial de Datos} Los modelos CAD originales pueden alcanzar decenas o cientos de megabytes, e incluso más en ensamblajes extensos, por lo que requieren limpieza, simplificación geométrica, \textit{baking} y empaquetado selectivo para ser viables en web.

\paragraph{Rendimiento en Tiempo Real} Mantener 30 FPS o más, equivalente a un \textit{frame time} de aproximadamente 33,33\,ms, exige controlar presupuesto geométrico, materiales, \textit{draw calls} y estados de render.

\paragraph{Latencia de Interacción} Alternativas como \textit{Pixel Streaming} pueden ofrecer render remoto de alta calidad visual, pero introducen dependencia de infraestructura, red y latencias poco adecuadas para inspección técnica precisa (Epic Games, s.\,f.-a, s.\,f.-b).

\begin{figure}[tp]
\caption{Dimensiones del Núcleo Problemático del Proyecto}
\label{fig:nucleo_problemico_dimensiones}
\centering
\includegraphics[width=0.92\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter1/fig_1_nucleo_problematico_dimensiones_trimmed.pdf}
\apanote[14.5pt]{La figura resume los cuatro frentes que condicionan la viabilidad del prototipo: huella inicial de datos, rendimiento en tiempo real, latencia de interacción y fragmentación semántica. Elaboración propia.}
\end{figure}

\paragraph{Fragmentación de Datos Técnicos} La geometría del producto no basta para explicar su función, mantenimiento o evolución. Para que una representación digital avance hacia \textit{digital shadow} o \textit{digital twin}, debe existir una relación trazable entre piezas, metadatos, procedimientos, sensores, eventos y decisiones de soporte (Kritzinger et al., 2018; Digital Twin Consortium, 2020).
...
\paragraph{Pregunta 1} ¿Qué diferencias descriptivas se observan en el desempeño de tareas y la carga de trabajo percibida cuando usuarios con perfil afín realizan actividades de exploración e identificación del dron Holybro X500~V2 mediante un prototipo web 3D interactivo, en comparación con un soporte 2D de referencia? Adicionalmente, ¿bajo qué condiciones de rendimiento y memoria resulta técnicamente viable dicho prototipo en el navegador?

\paragraph{Pregunta 2} ¿Cuál es el nivel de usabilidad percibida del prototipo 3D medido con SUS tras su uso?
...
\centering
\includegraphics[width=\textwidth,height=0.45\textheight,keepaspectratio]{figures/chapter1/fig_1_alcance_visual_product_twin_trimmed.pdf}
\apanote[14.4pt]{La versión final se posiciona como \textit{visual product twin}: integra inspección, selección y preparación semántica, pero no telemetría real, sincronización operacional ni modelos calibrados. Elaboración propia.}
\end{figure}

\subsection*{Trade-off Explícito entre Tooling y Huella Inicial}
...
\subsection*{Ejecución de Lógica Compilada en WebAssembly}
...
\subsection*{\textit{Pipeline} Integrado de Importación y Renderizado}
...
\subsection*{Capacidad de Extensión para Visualización Técnica}
...
\subsection*{Compatibilidad Esperada de la Plataforma Web}
...
El proyecto aporta al programa por al menos cuatro vías principales: documenta un \textit{pipeline} técnico para llevar activos CAD a un visor web interactivo; articula computación gráfica, experiencia de usuario, documentación técnica y arte técnico en un mismo caso aplicado; convierte restricciones de rendimiento web en criterios medibles de diseño y optimización; y consolida un caso de estudio pertinente para la Ingeniería Multimedia, donde la claridad técnica del ensamblaje es tan importante como la calidad visual del resultado.

\section{Objetivos}

Los objetivos que guiaron la ejecución del presente trabajo de grado se definen a continuación.
...
Diseñar un \textit{pipeline} de optimización de activos 3D que reduzca la carga poligonal de modelos CAD originales hasta un presupuesto geométrico compatible con WebGL, preservando la legibilidad formal del ensamblaje, la identificación correcta de piezas y la lectura espacial suficiente para inspección técnica mediante retopología, limpieza de mallas y \textit{baking} de texturas.

Integrar en Unity URP un sistema de materiales y modos de visualización que combine renderizado realista con herramientas de inspección técnica, procurando un \textit{frame time} igual o inferior a 33,33\,ms (30 FPS o más) bajo el entorno de prueba controlado definido para la validación del \textit{build} web.
...
\paragraph{Modelo Único} El prototipo se desarrolló sobre un único modelo de referencia, aunque la arquitectura quedó preparada para generalizar catálogos, datos y saneamiento de runtime.

\paragraph{No Constituye un Digital Twin Operacional} La versión final no recibe telemetría real, no sincroniza estado con un activo físico, no ejecuta diagnóstico de mantenimiento ni se integra con sistemas PLM, ERP, CMMS, SCADA o IoT. Su alcance correcto es el de una plataforma visual-semántica preparada para evolucionar hacia esas capas.

\paragraph{Plataforma Web} El producto se concentró en la plataforma web compilada con WebAssembly. No se desarrollaron \textit{builds} nativas independientes de escritorio o móvil.
...
\paragraph{Herramientas Ocultas o No Integradas} Algunas capacidades permanecen en código, pero no forman parte de la UI final visible ni del flujo funcional documentado como estado final del producto.
